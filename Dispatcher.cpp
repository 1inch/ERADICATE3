...
Dispatcher::OpenCLException::OpenCLException(const std::string s, const cl_int res) :
    std::runtime_error(s + " (res = " + std::to_string(res) + ")"),
    m_res(res)
{
...
cl_command_queue Dispatcher::Device::createQueue(cl_context & clContext, cl_device_id & clDeviceId) {
    // nVidia CUDA Toolkit 10.1 only supports OpenCL 1.2 so we revert back to older functions for compatibility
...
void Dispatcher::Device::freeResources() {
    if (m_clQueue) {
        std::cout << "Releasing command queue for device " << m_index << std::endl;
        clReleaseCommandQueue(m_clQueue);
        m_clQueue = nullptr;
    }
    if (m_kernelIterate) {
        std::cout << "Releasing kernel for device " << m_index << std::endl;
...
void CL_CALLBACK Dispatcher::staticCallback(cl_event event, cl_int event_command_exec_status, void * user_data) {
    if (event_command_exec_status != CL_COMPLETE) {
        throw std::runtime_error("Dispatcher::onEvent - Got bad status");
    }
